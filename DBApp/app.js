    ...
    db.pool.query(query2, function (error, rows, fields) {
      ...
      db.pool.query(query3, function (error, rows, fields) {
        ...
        data = data.map((itemordered) => {
          return Object.assign(itemordered, {
            orderID: ordersmap[itemordered.orderID],
          });
        });

        data = data.map((itemordered) => {
          return Object.assign(itemordered, {
            menuID: menusmap[itemordered.menuID],
          });
        });
    ...
    db.pool.query(query2, function (error, rows, fields) {
      ...
      db.pool.query(query3, function (error, rows, fields) {
        ...
        data = data.map((orderEmployee) => {
          return Object.assign(orderEmployee, {
            orderID: ordersmap[orderEmployee.orderID],
          });
        });

        data = data.map((orderEmployee) => {
          return Object.assign(orderEmployee, {
            employeeID: employeesmap[orderEmployee.employeeID],
          });
        });
    ...
    db.pool.query(query2, function (error, rows, fields) {
      ...
      data = data.map((order) => {
        return Object.assign(order, {
          customerID: customermap[order.customerID],
        });
      });
      ...
      db.pool.query(query3, function (error, rows, fields) {
        ...
        data = data.map((recipe) => {
          return Object.assign(recipe, {
            inventoryID: inventorymap[recipe.inventoryID],
          });
        });
...
app.put('/put-employee-ajax', function (req, res, next) {
  ...
  db.pool.query(
    updateEmployee,
    [name, phone, email, address, role, employeeID],
    function (error, rows, fields) {
      if (error) {
        // Log the error to the terminal so we know what went wrong, and send the visitor an HTTP response 400 indicating it was a bad request.
        console.log(error);
...
app.put('/put-order-ajax', function (req, res, next) {
  ...
  db.pool.query(
    updateOrder,
    [customerID, totalPrice, orderOn, orderID],
    function (error, rows, fields) {
      if (error) {
        console.log(error);
        res.sendStatus(400);
...
app.put('/put-inventory-ajax', function (req, res, next) {
  ...
  db.pool.query(
    updateInventory,
    [item, type, quantity, inventoryID],
    function (error, rows, fields) {
      if (error) {
        console.log(error);
        res.sendStatus(400);
...
app.put('/put-recipe-ajax', function (req, res, next) {
  ...
  db.pool.query(
    updateRecipe,
    [menu, inventory, reqAmt, recipeID],
    function (error, rows, fields) {
      if (error) {
        console.log(error);
        res.sendStatus(400);
...
app.listen(PORT, function () {
  ...
  console.log(
    'Express started on http://flip2.engr.oregonstate.edu:' +
      PORT +
